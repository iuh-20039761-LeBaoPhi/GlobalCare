...
function redirect_with_message(bool $ok, string $message, string $page = 'sua-thong-tin-khach-hang.php'): void
{
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: sua-thong-tin-khach-hang.php');
    exit;
}

$user = session_user_require_customer('../login.html', 'khach_hang/sua-thong-tin-khach-hang.php');
$customerId = (int)($user['id'] ?? 0);
if ($customerId <= 0) {
    redirect_with_message(false, 'Khong tim thay id khach hang trong session.');
}

// Doc du lieu dung ten cot bang khachhang.
...
$avatar = upload_or_keep_image('anh_dai_dien', 'assets/khachhang/anhdaidien', $existingAvatar);
if (!$avatar['success']) {
    redirect_with_message(false, (string)($avatar['message'] ?? 'Khong the upload anh dai dien.'));
}

$front = upload_or_keep_image('cccd_mat_truoc', 'assets/khachhang/cccdmattruoc', $existingFront);
if (!$front['success']) {
    redirect_with_message(false, (string)($front['message'] ?? 'Khong the upload CCCD mat truoc.'));
}

$back = upload_or_keep_image('cccd_mat_sau', 'assets/khachhang/cccdmatsau', $existingBack);
if (!$back['success']) {
...
header('Location: thong-tin-khach-hang.php?ok=1&msg=' . rawurlencode('Da cap nhat thong tin khach hang thanh cong.'));
exit;
