...
const userManager = (function () {
    ...
    function splitServiceIds(value) {
        return window.adminApi?.splitServiceIds ? window.adminApi.splitServiceIds(value) : [];
    }
    ...
    function normalizeUserRecord(row) {
        ...
        const normalizedUser = {
            ...
            dia_chi_doanh_nghiep: normalizeText(row?.dia_chi_doanh_nghiep || row?.diachidonvi || ""),
            loai_phuong_tien: normalizeText(row?.loai_phuong_tien || row?.vehicle_type || ""),
            note_admin: normalizeText(row?.note_admin || ""),
            link_avatar: normalizeText(row?.link_avatar || row?.avatar_link || ""),
            link_cccd_truoc: normalizeText(row?.link_cccd_truoc || row?.cccd_front_link || ""),
            link_cccd_sau: normalizeText(row?.link_cccd_sau || row?.cccd_back_link || ""),
    ...
    function renderFilterChips() {
        ...
        if (state.filters.role) {
            chips.push({ key: "role", label: `Vai trò: ${getRoleLabel(state.filters.role)}` });
        }
        if (state.filters.status) {
            chips.push({ key: "status", label: `Trạng thái: ${getStatusMeta(state.filters.status).label}` });
        }
        ...
        container.innerHTML = chips
            .map((chip) => `<div class="chip">${escapeHtml(chip.label)} <i class="fas fa-times close" onclick="userManager.clearFilter('${escapeHtml(chip.key)}')"></i></div>`)
            .join("");
    }
    ...
    function applyFilters() {
        ...
        state.filteredUsers = state.allUsers.filter((user) => {
            ...
            if (state.filters.status && user.resolved_status !== state.filters.status) {
                return false;
            }
            if (state.filters.verification === "complete" && !user.verification_complete) {
                return false;
            }
            if (state.filters.verification === "missing" && user.verification_complete) {
                return false;
            }
    ...
    function renderTable() {
        ...
        if (!state.filteredUsers.length) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:40px; color:var(--slate-light);">Không có tài khoản phù hợp bộ lọc hiện tại.</td></tr>';
            return;
        }
    ...
    function setFormValues(user = null) {
        ...
        document.getElementById("modalTitle").textContent = user ? `Cập nhật ${user.hovaten || user.sodienthoai || user.id}` : "Tạo tài khoản mới";
        document.getElementById("passwordField").querySelector("label").textContent = user ? "Mật khẩu mới (để trống nếu giữ nguyên)" : "Mật khẩu";
    ...
    function showUserModal(id = "") {
        const user = state.allUsers.find((item) => String(item.id) === String(id)) || null;
        setFormValues(user);
    ...
    function getServiceIdForRole(role, existingUser = null) {
        const existingIds = splitServiceIds(existingUser?.id_dichvu || "").filter((serviceId) => serviceId !== "0");
        ...
        const remainingIds = existingIds.filter((serviceId) => serviceId !== movingServiceId);
    ...
    async function handleSubmit(event) {
        ...
        const existingUser = state.allUsers.find((item) => String(item.id) === String(id)) || null;
        ...
        const payload = {
            ...
            dia_chi_doanh_nghiep: normalizeText(document.getElementById("dia_chi_doanh_nghiep").value),
            loai_phuong_tien: normalizeText(document.getElementById("loai_phuong_tien").value),
            ...
            link_cccd_truoc: normalizeText(document.getElementById("link_cccd_truoc").value),
            link_cccd_sau: normalizeText(document.getElementById("link_cccd_sau").value),
    ...
    async function quickUpdateStatus(id, status) {
        try {
            await window.adminApi.update(window.adminApi.USERS_TABLE, {
                trangthai: status,
                updated_at: new Date().toISOString(),
            }, id);
    ...
    function handleFilterChange() {
        ...
        state.filters.verification = document.getElementById("verificationFilter").value;
        applyFilters();
    }
    ...
    function handleDelete(id) {
        ...
        confirmButton.onclick = async function () {
            ...
            try {
                await window.adminApi.delete(window.adminApi.USERS_TABLE, state.userIdToDelete);
    ...
    async function fetchUsers() {
        ...
        try {
            ...
            showToast(error?.message || "Không thể tải dữ liệu người dùng.", "danger");
        }
    }
