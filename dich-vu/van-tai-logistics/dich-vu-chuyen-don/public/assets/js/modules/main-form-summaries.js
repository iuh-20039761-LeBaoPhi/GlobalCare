...
function formatConfiguredSummaryParts(
  scope,
  items,
  deps,
  fallback = "Chưa có",
...
function formatBookingDeploymentDetail(
  scope,
  serviceValue,
  deps,
  summaryConfig,
...
function formatBookingDistance(scope, deps) {
  ...
  const fromLat = Number(
    scope.querySelector("[data-ban-do-dat-lich-toa-do='diem_di_lat']")?.value ||
      0,
  );
  const fromLng = Number(
    scope.querySelector("[data-ban-do-dat-lich-toa-do='diem_di_lng']")?.value ||
      0,
  );
  const toLat = Number(
...
function updateBookingSummarySectionVisibility(summaryBox) {
  summaryBox.querySelectorAll("[data-hide-when-empty]").forEach((section) => {
    ...
    const hasVisibleMedia = !section.querySelector("[data-media-dat-lich-luoi]")
      ?.hidden;
